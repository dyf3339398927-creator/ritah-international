...
function render(){if(!initialized){options($('locale'),Object.entries(state.regions),r=>r[0],r=>r[1]);$('interval').value=state.interval;regionChanged();initialized=true;}
 ...
 for(const t of state.targets){const old=seen.get(t.id);if(t.status==='in_stock'&&old!=='in_stock'&&old!==undefined){beep();toast(t.storeName+'：'+t.name+' 可取货');}seen.set(t.id,t.status);
   ...
   const controls=node('div',undefined,'target-actions'),background=node('button','后台加购 · 优先','preferred-buy'),visible=node('button',state.container?'可见助手 · 原生版':'可见助手 ↗'),link=node('a','官网商品'),remove=node('button','移除'),modes=state.purchaseModes||['headless','visible'];
   background.disabled=!modes.includes('headless');background.onclick=()=>{if(confirm(`确认在后台浏览器中为“${t.name} / ${t.part}”执行一次加入购物袋？`))action('purchase',{id:t.id,mode:'headless'});};visible.disabled=!modes.includes('visible');visible.title=state.container?'Docker 无法在宿主机显示浏览器窗口，请使用原生版':'';visible.onclick=()=>action('purchase',{id:t.id,mode:'visible'});link.href=t.url;link.target='_blank';link.rel='noreferrer';remove.onclick=()=>action('remove',{id:t.id});if(state.purchaseAvailable!==false)controls.append(background,visible);controls.append(link,remove);card.append(controls,node('small',t.checkedAt?'上次查询 '+new Date(t.checkedAt*1000).toLocaleTimeString():'等待首次查询'));list.append(card);
 }
 $('purchaseState').textContent=state.purchase.message||(state.container?'Docker 优先使用内置无头 Chromium；可见助手需使用原生版。':'默认优先后台加购；需要登录、验证码或人工选择时使用可见助手。');
