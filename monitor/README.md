...
完整的逐步操作、双浏览器模式说明和界面截图请参阅 [详细使用说明](docs/USAGE.md)。
...
5. 每个目标提供两种购买方式，默认优先“后台加购”：经本地页面确认后，在不可见的独立浏览器会话中核对 SKU 并尝试加入购物袋一次；“可见助手”打开独立 Edge / Chrome 窗口，检查型号及选项后点击悬浮确认按钮。取货门店、登录和付款由你在官网完成。后台模式无法处理登录、验证码或需要人工选择的页面，此时应改用可见助手。Docker 内置无头 Chromium，但可见助手仅限原生版。无法确认商品时显示需要核对，不会报告购买成功。
...
GitHub Pages 只能托管静态网页，不能运行本后端或定时任务。分发 ZIP 给其他人后，每位用户在自己的电脑上启动服务。Docker 部署内置无头 Chromium，并将浏览器资料持久化到命名卷；Docker 与宿主机浏览器不共享登录或购物袋会话。尚未实现远程服务器多用户部署。
